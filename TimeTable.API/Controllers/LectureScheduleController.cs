...
namespace TimeTable.API.Controllers
{
    ...
    public class LectureScheduleController : ControllerBase
    {
        ...
        [HttpGet("CalendarJustArranged")]
        public async Task<MethodResult> GetCalendarJustArranged(int pageIndex, int pageSize, int check, string Name, int Count)
        {
            var result = await _lectureSchedureRepons.GetCalendarJustArrangedAsync(pageIndex, pageSize, check, Name, Count);
            if (result.Item1 == null) return MethodResult.ResultWithError(result.Item1, 400, "Error", result.Item2);
            return MethodResult.ResultWithSuccess(result.Item1, 200, "Successfull", result.Item2);
        }
        ...
        public async Task<MethodResult> UserRegisterEdCalendar (string token, Guid idSchedure, string Course_Code)
        {
            ...
            if(result != "Đăng ký lịch thành công") return MethodResult.ResultWithError(result, 400, "Error", 0);
